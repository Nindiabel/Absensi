...
class LivenessDetector:
    ...
    def is_live(self, image: np.ndarray, threshold: float = 0.8) -> Tuple[bool, float, float]:
        probs = self.detect(image)[0]
        ...
        try:
            import cv2
            from src.core.blink import get_ear
        ...
        # ===============================
        # EVALUASI LIVENESS PROBABILITY
        # ===============================
        if live_prob > threshold:
            return True, live_prob, ear

        return False, live_prob, ear
